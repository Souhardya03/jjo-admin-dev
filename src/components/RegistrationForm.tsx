...
export default function RegistrationForm() {
    ...
    const [verifyInviteCode] = useVerifyInviteCodeMutation();
    const { data: getEmailTemplate } = useGetTemplatesQuery({ search: "Welcome Mail" });
    const email_template = getEmailTemplate?.data[0];

    const { register, control, watch, reset, trigger, formState: { errors } } = useForm<RegisterFormData>({
    ...
    const watchInviteCode = watch("Invite_Code");
        ...
        const fetchSecret = async () => {
            ...
            try {
                const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/stripe`, {
                    ...
                    body: JSON.stringify({ amount: watchAmount, email: watchEmail }),
                });
    ...
    }, [watchAmount, watchEmail]);

    const syncToDatabase = async (transactionId: string) => {
        const values = watch();
    ...
    const handleFinalSubmit = async (stripe: any, elements: any) => {
        ...
        try {
            ...
            if (paymentStatus === "succeeded") {
                await syncToDatabase(currentIntent?.id || "");
            }
        } catch (err: any) {
            toast.error((err as any).data.message || "Process failed.");
        } finally {
            setIsLoopingCreation(false);
        }
    };
                    ...
                    <form className="space-y-8">
                            ...
                            <div className="p-8 space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <Input {...register("Invite_Code")} placeholder="6 Digits" maxLength={6} className={cn("h-11 border-[#eaddc7] uppercase font-mono", errors.Invite_Code && "border-red-500")} />
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="space-y-1.5">
                                        ...
                                        <Input {...register("PhoneNo")} className={cn("h-11 border-[#eaddc7]", errors.PhoneNo && "border-red-500")} placeholder="Enter phone" />
                                        <FormError message={errors.PhoneNo?.message} />
                                    </div>
                                    <div className="space-y-1.5">
                                        <Label className="font-semibold text-xs uppercase text-[#6b5f52]">Email Address</Label>
                                        <Input {...register("EmailAddress")} type="email" className={cn("h-11 border-[#eaddc7]", errors.EmailAddress && "border-red-500")} placeholder="johndoe@gmail.com" />
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    <div className="space-y-1.5">
                                        ...
                                        <Controller control={control} name="Gender" render={({ field }) => (
                                            <Select onValueChange={field.onChange} value={field.value}>
                                                <SelectTrigger className={cn("h-11 w-full border-[#eaddc7]", errors.Gender && "border-red-500")}><SelectValue placeholder="Select" /></SelectTrigger>
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <Controller control={control} name="DOB" render={({ field }) => (
                                            <Popover>
                                                <PopoverTrigger asChild><Button variant="outline" className={cn("w-full h-11 border-[#eaddc7] justify-start text-xs font-normal", errors.DOB && "border-red-500")}><CalendarIcon className="mr-2 h-3 w-3 text-[#b49157]" />{field.value ? format(field.value, "MM/dd/yyyy") : "MM/DD/YYYY"}</Button></PopoverTrigger>
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <Controller control={control} name="Activity" render={({ field }) => (
                                            <Select onValueChange={field.onChange} value={field.value}>
                                                <SelectTrigger className={cn("h-11 w-full border-[#eaddc7]", errors.Activity && "border-red-500")}><SelectValue placeholder="Select" /></SelectTrigger>
                                                <SelectContent>
                                ...
                                {primaryActivity === "Other" && <div className="space-y-1.5"><Input {...register("OtherActivity")} placeholder="Specify activity" className={cn("h-11 border-[#b49157]/40", errors.OtherActivity && "border-red-500")} /><FormError message={errors.OtherActivity?.message} /></div>}
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-2xl shadow-sm border border-[#eaddc7] overflow-hidden">
                            ...
                            <div className="p-8 space-y-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div className="space-y-1.5">
                                        <Input {...register("City")} placeholder="City" className={cn("h-11 border-[#eaddc7]", errors.City && "border-red-500")} />
                                        <FormError message={errors.City?.message} />
                                    </div>
                                    <div className="space-y-1.5">
                                        <Input {...register("State")} placeholder="State" className={cn("h-11 border-[#eaddc7]", errors.State && "border-red-500")} />
                                        <FormError message={errors.State?.message} />
                                    </div>
                                    <div className="space-y-1.5">
                                        <Input {...register("Zip")} placeholder="Zip" className={cn("h-11 border-[#eaddc7]", errors.Zip && "border-red-500")} />
                                        <FormError message={errors.Zip?.message} />
                                    </div>
                                </div>
                            </div>
                        </div>

                        {/* ADDITIONAL MEMBERS (Same as Original) */}
                                ...
                                {fields.map((field, index) => (
                                    <div key={field.id} className="group relative p-6 bg-white rounded-2xl border border-[#eaddc7] space-y-4 shadow-sm">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
                                            ...
                                            <div className="space-y-1.5"><Label className="text-[10px] uppercase font-bold text-[#b49157]">Email (Optional)</Label><Input placeholder="email@example.com" {...register(`familyMembers.${index}.Email`)} className={cn("h-10 border-[#eaddc7]", errors.familyMembers?.[index]?.Email && "border-red-500")} /><FormError message={errors.familyMembers?.[index]?.Email?.message} /></div>
                                            <div className="space-y-1.5"><Label className="text-[10px] uppercase font-bold text-[#b49157]">Activity</Label><Controller control={control} name={`familyMembers.${index}.Activity`} render={({ field }) => (
                                                <Select onValueChange={field.onChange} value={field.value}><SelectTrigger className={cn("h-10 w-full border-[#eaddc7]", errors.familyMembers?.[index]?.Activity && "border-red-500")}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Dance">Dance</SelectItem><SelectItem value="Singing">Singing</SelectItem><SelectItem value="Musical instruments">Musical Instruments</SelectItem><SelectItem value="Recitation">Recitation</SelectItem><SelectItem value="Other">Other</SelectItem></SelectContent></Select>
                                            )} /><FormError message={errors.familyMembers?.[index]?.Activity?.message} /></div>
                                            <div className="space-y-1.5"><Label className="text-[10px] uppercase font-bold text-[#b49157]">Gender</Label><Controller control={control} name={`familyMembers.${index}.Gender`} render={({ field }) => (
                                                <Select onValueChange={field.onChange} value={field.value}><SelectTrigger className={cn("h-10 w-full border-[#eaddc7]", errors.familyMembers?.[index]?.Gender && "border-red-500")}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="M">Male</SelectItem><SelectItem value="F">Female</SelectItem></SelectContent></Select>
                                            )} /><FormError message={errors.familyMembers?.[index]?.Gender?.message} /></div>
                                            <div className="space-y-1.5"><Label className="text-[10px] uppercase font-bold text-[#b49157]">DOB</Label><Controller control={control} name={`familyMembers.${index}.DOB`} render={({ field }) => (
                                                <Popover><PopoverTrigger asChild><Button variant="outline" className={cn("w-full h-10 border-[#eaddc7] justify-start text-[10px] px-2 font-normal", errors.familyMembers?.[index]?.DOB && "border-red-500")}><CalendarIcon className="mr-2 h-3 w-3 text-[#b49157]" />{field.value ? format(field.value, "MM/dd/yy") : "Select"}</Button></PopoverTrigger><PopoverContent className="p-0"><Calendar mode="single" selected={field.value} onSelect={field.onChange} captionLayout="dropdown-years" fromYear={1920} toYear={2026} /></PopoverContent></Popover>
                                            )} /><FormError message={errors.familyMembers?.[index]?.DOB?.message} /></div>
                                        </div>
                                        {familyMembersWatch?.[index]?.Activity === "Other" && <div className="space-y-1.5"><Input {...register(`familyMembers.${index}.OtherActivity`)} placeholder="Specify activity" className={cn("h-10 border-[#b49157]/40", errors.familyMembers?.[index]?.OtherActivity && "border-red-500")} /><FormError message={errors.familyMembers?.[index]?.OtherActivity?.message} /></div>}
                                        <div className="pt-2 border-t border-[#eaddc7]/50">
                                            <div className="flex items-center space-x-2 py-2">
                                                <Controller control={control} name={`familyMembers.${index}.SameAddress`} render={({ field }) => (<Checkbox id={`same-address-${index}`} checked={field.value} onCheckedChange={field.onChange} />)} />
                                            ...
                                            {!familyMembersWatch?.[index]?.SameAddress && (
                                                <div className="mt-4 space-y-4">
                                                    <div className="space-y-1.5">
                                                        <Input {...register(`familyMembers.${index}.Street`)} placeholder="Street Address" className={cn("h-10 border-[#eaddc7]", errors.familyMembers?.[index]?.Street && "border-red-500")} />
                                                        <FormError message={errors.familyMembers?.[index]?.Street?.message} />
                                                    </div>
                                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                                        <div className="space-y-1.5">
                                                            <Input {...register(`familyMembers.${index}.City`)} placeholder="City" className={cn("h-10 border-[#eaddc7]", errors.familyMembers?.[index]?.City && "border-red-500")} />
                                                            <FormError message={errors.familyMembers?.[index]?.City?.message} />
                                                        </div>
                                                        <div className="space-y-1.5">
                                                            <Input {...register(`familyMembers.${index}.State`)} placeholder="State" className={cn("h-10 border-[#eaddc7]", errors.familyMembers?.[index]?.State && "border-red-500")} />
                                                            <FormError message={errors.familyMembers?.[index]?.State?.message} />
                                                        </div>
                                                        <div className="space-y-1.5">
                                                            <Input {...register(`familyMembers.${index}.Zip`)} placeholder="Zip" className={cn("h-10 border-[#eaddc7]", errors.familyMembers?.[index]?.Zip && "border-red-500")} />
                                                            <FormError message={errors.familyMembers?.[index]?.Zip?.message} />
                                                        </div>
                                                    </div>
                                                </div>
                                            )}
                        ...
                        <div className="bg-[#f3ede4] rounded-2xl border-2 border-[#b49157]/20 p-8 space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <button type="button" onClick={() => setActiveMethod("card")} className={cn("flex items-center justify-center gap-3 py-4 rounded-xl border-2 transition-all", activeMethod === "card" ? "bg-white border-[#b49157] text-[#b49157] shadow-sm" : "bg-white/50 border-[#eaddc7] text-[#8c7e6d]")}>
                            ...
                            {activeMethod && (
                                <>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                                        <div className="space-y-1.5"><Label className="text-[#6b5f52] font-semibold text-xs uppercase">Amount</Label><Input {...register("Amount")} type="number" className={cn("h-11 bg-white border-[#eaddc7]", errors.Amount && "border-red-500")} placeholder="0.00" /><FormError message={errors.Amount?.message} /></div>
                                        <div className="space-y-1.5"><Label className="text-[#6b5f52] font-semibold text-xs uppercase">Date</Label><Controller control={control} name="TransactionDate" render={({ field }) => (
                                            <Popover><PopoverTrigger asChild><Button variant="outline" className={cn("w-full h-11 bg-white border-[#eaddc7] justify-start text-xs font-normal", errors.TransactionDate && "border-red-500")}><CalendarIcon className="mr-2 h-3 w-3 text-[#b49157]" />{field.value ? format(field.value, "PPP") : "Select Date"}</Button></PopoverTrigger><PopoverContent className="p-0"><Calendar mode="single" selected={field.value} onSelect={field.onChange} fromYear={2024} toYear={2026} /></PopoverContent></Popover>
                                        )} /><FormError message={errors.TransactionDate?.message} /></div>
                                    </div>

                                    {/* DYNAMIC PAYMENT UI */}
                                    <div className="pt-6 mt-6 border-t border-[#b49157]/10">
                                        {activeMethod === "card" ? (
                                            clientSecret ? (
                                                <Elements stripe={stripePromise} options={{ clientSecret, appearance: { theme: 'flat', variables: { colorPrimary: '#b49157' } } }}>
                                                    <StripeSubmitSection isProcessing={isLoopingCreation} onConfirm={(s, e) => handleFinalSubmit(s, e)} />
                                                </Elements>
                                            ) : (
                                                <div className="bg-white/50 p-10 rounded-xl border border-dashed border-[#eaddc7] flex flex-col items-center justify-center text-center">
                                                    {loadingSecret ? <><Loader2 className="w-8 h-8 animate-spin text-[#b49157] mb-2" /><p className="text-xs text-[#8c7e6d]">Initializing Stripe...</p></> : <><ShieldCheck className="w-8 h-8 text-[#eaddc7] mb-2" /><p className="text-xs text-[#8c7e6d]">Enter profile details and amount to pay by card.</p></>}
                                                </div>
                                            )
                                        ) : (
                                            <div className="animate-in w-full flex flex-col items-center justify-center fade-in duration-500">
                                                <div className="w-full ml-8" style={{ minWidth: "100%" }}>
                                                    ...
                                                    <PayPalButtons
                                                        className="w-full"
                                                        style={{ layout: "vertical", color: "gold", shape: "pill", height: 40 }}
                                                        forceReRender={[watchAmount]}
                                                        createOrder={async (data, actions) => {
                                                            ...
                                                            if (!watchAmount || !watchInviteCode) { toast.error("Enter Amount and Invite Code first"); return Promise.reject(); }
                                                            return actions.order.create({
                                                                purchase_units: [{ amount: { currency_code: "USD", value: watchAmount } }],
                                                                intent: "CAPTURE"
                                                            });
                                                        }}
                                                        onApprove={async (data, actions) => {
                                                            if (actions.order) {
                                                                ...
                                                                setIsLoopingCreation(true);
                                                                await syncToDatabase(details.id || "");
                                                                setIsLoopingCreation(false);
                                                            }
                                                        }}
                                                    />
